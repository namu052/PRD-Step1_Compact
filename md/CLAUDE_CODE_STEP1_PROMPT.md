# Claude Code 프롬프트: AI 지방세 지식인 APP - Step 1 (프론트엔드)
...
```markdown
# AI 지방세 지식인 APP - Step 1 Todolist

## 🔧 1단계: 프로젝트 초기화 + Mock 데이터
...
- [ ] mockCerts.json 생성 (가상 GPKI 인증서 2건)
- [ ] mockChatResponses.json 생성 (취득세 감면, 재산세 납부 2건)
- [ ] mockSources.json 생성 (출처 3건)
- [ ] MSW handlers.js 작성 (POST /api/auth/gpki, POST /api/chat, GET /api/preview/:id)
- [ ] MSW browser.js 설정 + main.jsx에서 개발 모드 시 MSW 시작

### ✅ 1단계 확인사항
- [ ] `npm run dev` 실행 시 에러 없이 빈 페이지가 뜨는가?
- [ ] 브라우저 콘솔에 "[MSW] Mocking enabled" 메시지가 표시되는가?
- [ ] 브라우저 콘솔에서 `fetch('/api/auth/gpki', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({cert_id:'cert_001', password:'test1234'})}).then(r=>r.json()).then(console.log)` 실행 시 `{success: true, ...}` 응답이 오는가?
...
- [ ] 인증서 목록에 "홍길동 (OO시 세무과)", "김영희 (OO시 재무과)" 2건이 보이는가?
- [ ] 인증서 선택 후 비밀번호 "test1234" 입력 + 확인 → 모달 닫힘?
- [ ] 모달 닫힌 후 상단 바에 "홍길동님" 표시되는가?
...
### ✅ 5단계 최종 확인사항 (전체 플로우)
- [ ] APP 실행 → GPKI 모달 표시
- [ ] 인증서 선택 + "test1234" 입력 → 로그인 성공 → 모달 닫힘
- [ ] 상단 바에 "홍길동님" + "로그아웃" 버튼 표시
...
`vite.config.js`에 Tailwind 플러그인을 추가해:
...
**`src/mocks/handlers.js`** — 아래 3개 API를 Mock으로 구현해:

1. **`GET /api/auth/certs`**: mockCerts.json 반환
2. **`POST /api/auth/gpki`**: password가 "test1234"이면 성공, 아니면 실패
3. **`POST /api/auth/logout`**: { success: true } 반환
4. **`POST /api/chat`**: question에서 키워드("취득세", "재산세") 매칭 → SSE 형식 응답 반환
...
5. **`GET /api/preview/:sourceId`**: mockChatResponses의 sources에서 id 매칭하여 반환
...
3. 콘솔에 `[MSW] Mocking enabled` 확인
4. 콘솔에서 fetch 테스트:
```javascript
fetch('/api/auth/gpki', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({cert_id:'cert_001', password:'test1234'})}).then(r=>r.json()).then(console.log)
```
5. `{success: true, user_name: "홍길동", session_id: "..."}` 응답 확인
...
- 모달 외부 클릭으로 닫히지 않음
- 상단: "🔐 행정전자서명 인증 로그인" 제목
- 인증서 저장 위치: 라디오 버튼 ("하드디스크이동식" 기본 선택, "보안토큰")
- 인증서 목록: GET /api/auth/certs로 로드, 각 항목은 카드형태로 표시
  - 소유자명, 부서명, 유효기간, 시리얼번호
...
구현 후:
1. `npm run dev`로 실행
2. 브라우저에서 전체 플로우 테스트 (모달 표시 → 로그인 → 로그아웃)
...
구현 후:
1. `npm run dev`로 실행
2. 로그인 → "취득세 감면 대상" 입력 → 전체 스트리밍 플로우 확인
...
구현 후:
1. `npm run dev`로 실행
2. 로그인 → 질문 → 출처 표시 → 카드 클릭 → 상세 → 바로가기 → 닫기 전체 플로우 확인
...
2. **Mock 전용**: 이 Step에서는 실제 API 서버가 없음. MSW로만 동작.
