...
@pytest.mark.asyncio
async def test_verification_pipeline_normal():
...
@pytest.mark.asyncio
async def test_verification_pipeline_with_hallucination():
    crawl_results = _to_crawl_results(MOCK_CRAWL_RESULTS)
    ...
    verification = verification_aggregator.aggregate(sources, claims)
    assert verification.removed_claims


@pytest.mark.asyncio
async def test_verification_pipeline_all_wrong():
    crawl_results = _to_crawl_results(MOCK_CRAWL_RESULTS)
    sources = await source_verifier.verify(MOCK_DRAFT_ALL_WRONG, crawl_results)
